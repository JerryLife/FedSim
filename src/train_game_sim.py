...
load_cache = True
if load_cache:
    # with open("cache/game_sim_cat2i_appid.pkl", 'rb') as appid_file, \
    #      open("cache/game_sim_cat2i_steamid.pkl", 'rb') as steamid_file:
    #     cat2i_appid = pickle.load(appid_file)
    #     cat2i_steamid = pickle.load(steamid_file)

    print("Loading train dataset")
    trn_set = torch.load("cache/game_sim_train_dataset.pth")
    print("Loading validation dataset")
    vld_set = torch.load("cache/game_sim_val_dataset.pth")
    print("Loading test dataset")
    tst_set = torch.load("cache/game_sim_test_dataset.pth")
    print("Loading counts")
    counts = torch.load("cache/game_sim_counts.pth")

    print("Loading steam keys and labels")
    with open("cache/game_sim_steam_vld_label.pkl", 'rb') as vld_file, \
         open("cache/game_sim_steam_tst_label.pkl", 'rb') as tst_file:
        steam_vld_label = pickle.load(vld_file)
        steam_tst_label = pickle.load(tst_file)
    ...
    print("Loading steam keys and labels")
    steam_vld_df = pd.read_csv(root + "steam_data_val.csv")
    steam_tst_df = pd.read_csv(root + "steam_data_test.csv")
    ...
    torch.save(trn_set, "cache/game_sim_train_dataset.pth")
    print("Saving validation dataset")
    torch.save(vld_set, "cache/game_sim_val_dataset.pth")
    print("Saving test dataset")
    torch.save(tst_set, "cache/game_sim_test_dataset.pth")
    print("Saving counts")
    torch.save(counts, "cache/game_sim_counts.pth")
...
def test(data_loader, status):
    ...
    with torch.no_grad():
        for data in tqdm(data_loader, desc="Val" if status == 'vld' else "Test"):
            ...
            if status != 'trn':
                for appid, steamid, sim_score, pred in zip(keys[0].detach().cpu().numpy(), keys[1].detach().cpu().numpy(),
                                                           sim_scores, predicted):
                    key = (appid, steamid)
                    if (key in vld_pred_all and sim_score > vld_pred_all[key]) or key not in vld_pred_all:
                        vld_pred_all[key] = pred

    vld_loss /= vld_total_sample
    vld_acc_sample = vld_correct_sample / vld_total_sample
    if status != 'trn':
        if status == 'vld':
            labels_all = steam_vld_label
        else:   # status = 'tst'
            labels_all = steam_tst_label

        if len(labels_all) == len(vld_pred_all):
            print("Equal: {}".format(len(labels_all)))
        else:
            print("Not equal: labels_all={}, vld_pred_all={}".format(len(labels_all), len(vld_pred_all)))

        vld_correct = 0
        for k, v in vld_pred_all.items():
            if labels_all[k] == v:
                vld_correct += 1
        vld_acc = vld_correct / len(labels_all)

        if status == 'vld':
            scheduler.step(vld_loss)

        return vld_loss, vld_acc, vld_acc_sample
    else:
...
for epoch in range(epochs):
    ...
    vld_loss, vld_acc, vld_acc_sample = test(vld_loader, 'vld')
    print("Epoch %d: vld_loss: %.4f vld_acc: %.4f, vld_sample_acc: %.4f" % (epoch, vld_loss, vld_acc, vld_acc_sample))
    tst_loss, tst_acc, tst_acc_sample = test(tst_loader, 'tst')
    print("Epoch %d: tst_loss: %.4f tst_acc: %.4f, tst_sample_acc: %.4f" % (epoch, tst_loss, tst_acc, tst_acc_sample))
    end_t = time.time()
